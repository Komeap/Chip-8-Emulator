...
int cpu_cycle(struct CPU *cpu){
 ...
 switch (op) {
        ...
        case 0x3: if (cpu->Vx[x] == nn) cpu->PC += 2; break;
        case 0x4: if (cpu->Vx[x] != nn) cpu->PC += 2; break;
        case 0x5: if (cpu->Vx[x] == cpu->Vx[y]) cpu->PC += 2; break;
        case 0x6: cpu->Vx[x] = nn; break;
        case 0x7: cpu->Vx[x] += nn; break;

        case 0x8:
            switch (n) {
                case 0x0: cpu->Vx[x] = cpu->Vx[y]; break;
                case 0x1: cpu->Vx[x] |= cpu->Vx[y]; break;
                case 0x2: cpu->Vx[x] &= cpu->Vx[y]; break;
                case 0x3: cpu->Vx[x] ^= cpu->Vx[y]; break;
                case 0x4: {
                ...
                case 0x5:
                    ...
                    cpu->Vx[x] -= cpu->Vx[y];
                    break;
                case 0x6: // CHIP-8 Standard: shift Vx
                ...
                case 0x7:
                    ...
                    break;
                case 0xE: // CHIP-8 Standard: shift Vx
        ...
        case 0x9: if (cpu->Vx[x] != cpu->Vx[y]) cpu->PC += 2; break;
        case 0xA: cpu->I = nnn; break;
        case 0xB: cpu->PC = nnn + cpu->Vx[0]; break;
        case 0xC: cpu->Vx[x] = (rand() % 256) & nn; break;
        ...
        case 0xD: {
            ...
            uint8_t vf = 0;
            if (cpu->display) Display_DRW(cpu->display, &spr, cpu->Vx[x], cpu->Vx[y], &vf);
            cpu->Vx[0xF] = vf;
            Sprite_destroy(&spr);
        } break;
        ...
        case 0xF:
            switch (nn) {
                case 0x07: cpu->Vx[x] = cpu->DT; break;
                case 0x0A: // LD Vx, K (Wait for key)
                {
                    uint8_t key = 0;
                    // Keyboard_wait est bloquant, comme demandé par l'instruction
                    if (Keyboard_wait(cpu->keyboard, &key) == 0) {
                         cpu->Vx[x] = key;
                    }
                }
                break;
                case 0x15: cpu->DT = cpu->Vx[x]; break;
                case 0x18: cpu->ST = cpu->Vx[x]; break;
                case 0x1E: cpu->I += cpu->Vx[x]; break;
                case 0x29: cpu->I = (cpu->Vx[x] & 0xF) * 5; break;
                case 0x33:
                    ...
                    break;
                case 0x55:
                    for (int i = 0; i <= x; i++) write_RAM(cpu->ram, cpu->I + i, cpu->Vx[i]);
                    break;
                case 0x65:
                    for (int i = 0; i <= x; i++) read_RAM(cpu->ram, cpu->I + i, &cpu->Vx[i]);
                    break;
            }
            break;
    }
